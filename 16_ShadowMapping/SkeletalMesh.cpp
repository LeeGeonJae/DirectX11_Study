...
void SkeletalMesh::ShadowDraw(ComPtr<ID3D11DeviceContext> deviceContext)
{
...
void SkeletalMesh::createVS()
{
	...
	assert(SUCCEEDED(hr));
	LoadShaderFromFile(L"SkeletalShadowShader.hlsl", "VS", "vs_5_0", m_vsShadowBlob);
...
void SkeletalMesh::createPS()
{
	...
	assert(SUCCEEDED(hr));
	LoadShaderFromFile(L"SkeletalShadowShader.hlsl", "PS", "ps_5_0", m_psShadowBlob);
