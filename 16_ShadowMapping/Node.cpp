...
void Node::Update(ComPtr<ID3D11DeviceContext> deviceContext, SimpleMath::Matrix modelTransform)
{
	...
	// 노드 위치 곱하기
	{
		...
		{
			m_CurrentTime += TimeManager::GetInstance()->GetfDT() * ImGuiMenu::ModelAnimationSpeed * 15.f;
	...
	if (m_Name == "mixamorig:RightToeBase")
	{
		int a = 0;
	}
...
void Node::ShadowDraw(ComPtr<ID3D11DeviceContext> deviceContext)
{
	for (auto mesh : m_Mesh)
	{
	...
	for (auto child : m_Children)
	{
		if (child != nullptr)
		{
			child->ShadowDraw(deviceContext);
		}
	}
}
