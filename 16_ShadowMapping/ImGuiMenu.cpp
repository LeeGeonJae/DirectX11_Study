...
float ImGuiMenu::CameraNearFar[2] = { 1.f , 3000000.0f };
...
Vector4 ImGuiMenu::DirectionLightDir = { 0.f, -0.7f, -0.7f, 0.0f };
...
int ImGuiMenu::FixedUpdateCount = 0;
int ImGuiMenu::CameraBeginOverlapCount = 0;
int ImGuiMenu::CameraOverlapCount = 0;
int ImGuiMenu::CameraEndOverlapCount = 0;
bool ImGuiMenu::IsBlockNotOverlap = true;

Vector3 ImGuiMenu::CameraBeforePosition = Vector3::Zero;
Vector3 ImGuiMenu::CameraAfterPosition = Vector3::Zero;
Vector3 ImGuiMenu::ShadowDirection = Vector3::Zero;

ID3D11ShaderResourceView* ImGuiMenu::ShadowMap = nullptr;
...
void ImGuiMenu::Render()
{
	...
	// Resource
	{
		...
		{
			ImGui::Checkbox("CollisionType", &IsBlockNotOverlap);
		}
		...
		str = to_string(FixedUpdateCount);
		ImGui::Text("Time : %s", str.c_str());
		...
		ImGui::Text("ObjectCount : %s", str.c_str());
		str = to_string(DrawComponentCount);
		ImGui::Text("DrawObjectCount : %s", str.c_str());
		str = to_string(CameraBeginOverlapCount);

		ImGui::Text("");

		ImGui::Text("CameraBeginOverlapCount : %s", str.c_str());
		str = to_string(CameraEndOverlapCount);
		ImGui::Text("CameraEndOverlapCount : %s", str.c_str());
		str = to_string(CameraOverlapCount);
		ImGui::Text("CameraOverlapCount : %s", str.c_str());
		str = to_string(CameraBeforePosition.x) + ", " + to_string(CameraBeforePosition.y) + ", " + to_string(CameraBeforePosition.z);
		ImGui::Text("CameraBeforePosition : %s", str.c_str());
		str = to_string(CameraAfterPosition.x) + ", " + to_string(CameraAfterPosition.y) + ", " + to_string(CameraAfterPosition.z);
		ImGui::Text("CameraAfterPosition : %s", str.c_str());
		str = to_string(ShadowDirection.x) + ", " + to_string(ShadowDirection.y) + ", " + to_string(ShadowDirection.z);
		ImGui::Text("ShadowDirection : %s", str.c_str());

		ImGui::Image(ShadowMap, ImVec2(256, 256));
