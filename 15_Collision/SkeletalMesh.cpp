...
void SkeletalMesh::Draw(ComPtr<ID3D11DeviceContext> deviceContext)
{
	...
	for (int i = 0; i < static_cast<int>(TextureType::END); i++)
	{
		...
		if (find != nullptr)
		{
			...
			DEVICE_CONTEXT->PSSetShaderResources(i, 1, find->m_Texture.GetAddressOf());
		}
		else
	...
	DEVICE_CONTEXT->UpdateSubresource(m_bisTextureMapBuffer.Get(), 0, nullptr, &m_CBIsValidTextureMap, 0, 0);
	DEVICE_CONTEXT->UpdateSubresource(m_MaterialBuffer.Get(), 0, nullptr, &m_CBMaterial, 0, 0);

	DEVICE_CONTEXT->VSSetConstantBuffers(4, 1, m_bisTextureMapBuffer.GetAddressOf());
	DEVICE_CONTEXT->PSSetConstantBuffers(4, 1, m_bisTextureMapBuffer.GetAddressOf());
	DEVICE_CONTEXT->VSSetConstantBuffers(5, 1, m_MaterialBuffer.GetAddressOf());
	DEVICE_CONTEXT->PSSetConstantBuffers(5, 1, m_MaterialBuffer.GetAddressOf());
	...
	DEVICE_CONTEXT->DrawIndexed(static_cast<UINT>(m_Indices.size()), 0, 0);
}
